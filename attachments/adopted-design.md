...
Every source reference below is a link opening at this note's own exact head.
None pins a revision, because a link naming another commit is refused as one
this record does not cite. The lines were read at main
`d1ae6ade996af7167f22a2170dfaa6a200e004e1`, which this branch does not modify.
The superseded design is absent from this head, so it is cited by revision as
`notes/2026-08-27-lane-identity.md@6131249c` and is not a link.
...
subcommand ([main.go:107](cmd/gs/main.go:107)), and no `gitseq.*` Git config key
...
([envelope.go:22](internal/intent/envelope.go:22)) and compares it byte for byte
with the signed intent ([kernel.go:1535](internal/kernel/kernel.go:1535)); a
...
([graph.go:46](internal/gitstore/graph.go:46)). Anyone who can write a commit can
...
input retires nothing by itself ([SKILL.md:539](SKILL.md:539)). Every
...
([app.go:616](internal/app/app.go:616)); the hardened trailer scan with NUL
framing and per object hash re verification
([graph.go:40](internal/gitstore/graph.go:40)); the bounded ref inventory and the
ancestry graph that answers unknown rather than false
([landing_graph.go:65](internal/app/landing_graph.go:65)); canonical resolution
with typed refusals naming at most eight candidates
([eventref.go:32](internal/eventref/eventref.go:32)); and the closed Git
environment allowlist ([app.go:336](internal/app/app.go:336)). New is one
association pass, taking an explicit ref set and depth limit instead of the fixed
newest 81 ([ui.go:60](internal/service/ui.go:60)), walking first parent lineage
...
- `corroborated`: a signed artifact statement names this exact commit, and the
  ...
  actor ([binding.go:228](internal/reviewguard/binding.go:228)), or, for self
  initiated work, as the adopted decision the same code already recognises
  ([402](internal/reviewguard/binding.go:402)).
...
([graph.go:36](internal/gitstore/graph.go:36)), so an attacker who copies a real
...
([worktree_landing.go:11](internal/app/worktree_landing.go:11),
[landing.go:37](internal/app/landing.go:37)), plus 4,096 refs, 256 branch tips,
...
([binding.go:402](internal/reviewguard/binding.go:402)). Refusals before any
...
([mergeplan.go:351](internal/mergeplan/mergeplan.go:351)); an existing branch not
asked for.
...
([mergeplan.go:967](internal/mergeplan/mergeplan.go:967)) it warns, names the
...
([config.go:420](internal/apphost/config.go:420)) already separates from the
common directory, written through the existing atomic file and flock pattern
([346](internal/apphost/config.go:346)). It holds the full canonical governing
...
[notes-and-decisions.md:238](notes/2026-08-21-notes-and-decisions.md:238) is
...
([app.go:356](internal/app/app.go:356)), a repository wide change altering config
...
readable ([359](internal/app/app.go:359)), so anything kept there stays an
...
([SKILL.md:440](SKILL.md:440)). A hook is optional, off by default, installed
...
([merge.md:236](docs/reference/gs/merge.md:236)).
...
([mergeplan.go:1084](internal/mergeplan/mergeplan.go:1084)). The hash is the
...
([gitstore.go:338](internal/gitstore/gitstore.go:338)), so competing creation
...
([landing.go:121](internal/workroom/landing.go:121)), resolved in the repository
...
([state.md:70](docs/reference/gs/state.md:70)), and this very request carries
...
the declared vocabulary ([kinds.go:22](internal/workroom/kinds.go:22)). It is
...
([schema.go:16](internal/workroom/schema.go:16)), replaying history once.
...
boundary ([app.go:178](internal/app/app.go:178)). The checkout label stays in the
ephemeral endpoint.
...
([worktree_landing.go:46](internal/app/worktree_landing.go:46)); containment and
...
([mergeplan.go:1563](internal/mergeplan/mergeplan.go:1563)), removed by the
process that created them by recorded path only
([gitseq-perf/main.go:833](cmd/gitseq-perf/main.go:833)). Nothing searches for
