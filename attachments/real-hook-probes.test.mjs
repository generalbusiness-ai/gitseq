...
// The endpoint has no profile/interpretability field. This exercises the real
// hook with its actual wire shape; it is not a live service/profile experiment.
test('an undifferentiated cold rebuild cannot establish retained-profile safety',async()=>fixture(async f=>{
 const before=f.get().status;
 await act(async()=>[...f.timers.values()][0]());
 await act(async()=>f.rebuilds[0](f.reply({running:true,verified:1,total:10})));
 console.log('status still exposed during undifferentiated rebuild:',f.get().status===before,document.body.textContent);
 assert.equal(f.get().status,undefined,'without evidence of profile compatibility the old projection must not remain available');
}));

// The separate actual Workspace profile-change probe observes running:false
// while checkpoint-backed interpretation is paused before publication.
test('profile rebuild without cold-audit progress leaves the old projection unqualified',async()=>fixture(async f=>{
 const before=f.get().status;
 await act(async()=>[...f.timers.values()][0]());
 await act(async()=>f.rebuilds[0](f.reply({running:false})));
 console.log('checkpoint profile rebuild:',f.get().status===before,f.get().rebuilding,document.body.textContent);
 assert.equal(f.get().status,undefined,'profile-invalidated retained projection must be unavailable before replacement publication');
}));
