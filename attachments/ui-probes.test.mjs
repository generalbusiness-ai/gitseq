...
async function fixture(body) {
 ...
 const timers=new Map(),waits=[],rebuilds=[];let latest;
 ...
 globalThis.fetch=(url)=>{
  ...
  if(url==='/v0/rebuild')return new Promise(resolve=>rebuilds.push(resolve));
 ...
 try {
  ...
  await body({timers,waits,rebuilds,status,reply,get:()=>latest});
