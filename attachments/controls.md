...
Thirty-three tests over three packages. The command-line tests drive the
...
## New controls after the second review (verdict 633a7890)

| Defect | Test |
|---|---|
| Recognized body fields signed as typed on all three surfaces | `cmd/gs` `TestRecognizedBodyFieldsResolveAndSignCanonicalIdentifiers` — `#N`, a short prefix, a whole hash and a canonical id across `authorizes_request`, `authorizes_approval`, `artifact` and `merge_approval`, asserted on the decoded signed body, with `authorizes_candidate` and free prose beside them untouched; plus no-match and ambiguous refusals with depth unchanged |
| the same through a batch entry body | `cmd/gs` `TestBatchEntryBodiesResolveTheirRecognizedFields` — signed body decoded, prose preserved, and a refusal naming the act and the field with nothing landed |
| the same through the MCP state tool | `cmd/gitseq-mcp` `TestStateToolResolvesRecognizedBodyFields` |
| the MCP refusal path dropped the disclosure | `cmd/gitseq-mcp` `TestStateToolDisclosesItsBases` now drives the real JSON-RPC loop through `failedToolResponse` and asserts the actual failure envelope: `isError`, the refusal as the first content block, and the warning as its own block beside it |
