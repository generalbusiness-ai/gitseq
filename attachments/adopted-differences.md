...
Source references are links that open at this note's own exact head, which is
why none pins a revision; the lines were read at main
...
cited by revision rather than linked.
...
([kernel.go:1535](internal/kernel/kernel.go:1535)). An implementing source
...
([binding.go:228](internal/reviewguard/binding.go:228)). A `claimed` association
...
controlled ([graph.go:36](internal/gitstore/graph.go:36)), so copying
...
([config.go:346](internal/apphost/config.go:346),
[420](internal/apphost/config.go:420)). It holds the full canonical governing
...
`extensions.worktreeConfig` ([app.go:356](internal/app/app.go:356)), a
...
([359](internal/app/app.go:359)), while a file in the private Git directory is
...
([merge.md:236](docs/reference/gs/merge.md:236)), precisely because a hostile
`pre-commit` hook can retarget `HEAD` ([main.go:946](cmd/gs/main.go:946)).
...
([landing-observations.md:85](docs/reference/landing-observations.md:85)) and
...
boundary and publishes basenames only ([app.go:178](internal/app/app.go:178),
[690](internal/app/app.go:690)). Recording paths durably would put one machine's
...
([landing.go:121](internal/workroom/landing.go:121)) and admitted as a typed
optional field of the declared vocabulary
([kinds.go:22](internal/workroom/kinds.go:22)). It is display and search only:
...
([state.md:70](docs/reference/gs/state.md:70),
[83](docs/reference/gs/state.md:83)), and this very request carries
...
1. **Bounded scan and cache invalidation.** The old text says the resident
   ...
   [landing.go:37](internal/app/landing.go:37)), plus a cache keyed on the
   durable frontier and the observed ref inventory digest.
2. **Self initiated work.** Recognition already exists at
   [binding.go:402](internal/reviewguard/binding.go:402)
   (`resolveSelfInitiated`) and [436](internal/reviewguard/binding.go:436)
...
3. **Trailers take the full canonical identifier only.**
   [SKILL.md:440](SKILL.md:440) already says so. Trailer assistance resolves the
...
5. **Schema and fold version cost.** Adding a validated `branch` to request,
   ...
   `ProfileVersion` ([schema.go:16](internal/workroom/schema.go:16), currently
...
6. **Naming.** "Lane" already names a work query lane
   ([query.go:23](internal/statusview/query.go:23)). New code says "checkout"
...
8. **Two settled word lists exist.**
   [mergeplan.go:967](internal/mergeplan/mergeplan.go:967)
   (`unsettledCommitment`) and
   [worktree_landing.go:46](internal/app/worktree_landing.go:46)
...
10. **Command environment.** `internal/app` runs Git under a closed environment
    allowlist ([app.go:336](internal/app/app.go:336)), while `cmd/gs`'s helper
    inherits the ambient environment ([main.go:3282](cmd/gs/main.go:3282)). A
...
12. **Scratch trees.** The existing patterns are the disposable clone with hooks
    emptied ([mergeplan.go:1563](internal/mergeplan/mergeplan.go:1563)) and the
    perf harness's own add and remove pair
    ([main.go:833](cmd/gitseq-perf/main.go:833),
    [839](cmd/gitseq-perf/main.go:839)). Reusing them is not new policy.
...
([schema.go:16](internal/workroom/schema.go:16)), which rejects every reader's
