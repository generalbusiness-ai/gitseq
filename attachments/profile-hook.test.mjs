...
test('a newly returned verified status restores availability after a profile drop',async()=>fixture(async f=>{
 await act(async()=>[...f.timers.values()][0]());
 await act(async()=>f.rebuilds[0](f.reply({running:false,profile:'app@fold-2'})));
 assert.equal(f.get().status,undefined);
 const next=f.status('new00000',2);next.profile='app@fold-2';
 await act(async()=>f.waits[0](f.reply({status:next})));
 assert.equal(f.get().status,next);
 assert.equal(f.waits.length,2);
 await act(async()=>[...f.timers.values()][0]());
 await act(async()=>f.rebuilds[1](f.reply({running:true,profile:'app@fold-2'})));
 assert.equal(f.get().status,next);
 assert.match(document.body.textContent,/Showing frontier new00000/);
}));
