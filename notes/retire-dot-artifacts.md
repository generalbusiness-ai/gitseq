...
This is a one-time migration, not part of the ordinary work loop. AGENTS.md
states the rule it enforces — never record a merge artifact at `.` — in two
sentences. Everything below is the procedure for clearing the artifacts
recorded there before that rule existed, and it lives here so that the file
every agent reads at the start of every session does not carry a runbook that
runs once.
