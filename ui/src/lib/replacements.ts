...
interface ReplacementProjection {
  ...
  statements: { event: string; retired?: boolean }[];
...
// names exactly one effective, unretired statement. Additional evidence and
// ambiguous candidates must not turn into a guessed UI link.
export function replacementForSupersede(act: SupersedeAct, projection: ReplacementProjection): string | undefined {
  ...
  const candidates = new Set(
    (projection.provenance[act.event] ?? []).filter((event) => {
      ...
      return event !== act.target && effective.has(event) && statement !== undefined && !statement.retired;
    }),
  );
