...
export function Thread({
  ...
  if (projection && index.has(root) && (!request || !spine)) {
    return <div className="flex min-h-0 flex-1 flex-col">
      <ThreadHeader title={`#${index.sequence(root) ?? "?"} · Durable record`} onBack={onBack} backLabel={backLabel} />
      <div className="overflow-y-auto p-4"><RecordDetail event={root} index={index} actors={projection.actors} tickets={tickets} nameOf={nameOf} onOpenThread={onOpenThread} /></div>
    </div>;
  }
  if (!projection || !request || !spine) {
    return (
      <div className="flex min-h-0 flex-1 flex-col">
        <ThreadHeader title="Gone" onBack={onBack} backLabel={backLabel} />
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <ThreadHeader title={`#${tickets.get(root) ?? "?"} · ${firstLine(request.text)}`} onBack={onBack} backLabel={backLabel} />
...
function ThreadHeader({ title, onBack, backLabel = "requests" }: { title: string; onBack: () => void; backLabel?: string }) {
  return (
