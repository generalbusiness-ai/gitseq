...
function reportedRoom({ verdict = "effective", retired = false } = {}) {
  const statements = [
    ...
    { event: "rep", sequence: 3, actor: "codex", kind: "report", text: "Done.", timestamp: AT, satisfier: "originating-requester", retired },
  ];
  return {
    decisions: [
      { event: "req", sequence: 1, verdict: "effective", reason: "recorded" },
...
async function withApp(projection, body) {
  ...
  globalThis.fetch = async (url, options) => {
    ...
    if (path.includes("/v0/status")) return reply(status(projection));
    if (path.includes("/v0/act")) {
      filed.push(JSON.parse(options?.body ?? "{}"));
    ...
    // A promise that never settles does neither.
    if (path.includes("/v0/wait")) return new Promise(() => {});
...
test("App files a ratification when the target's standing is good", async () => {
  // The positive control for the two below. If this stops filing, "zero
  // calls" there has stopped meaning anything.
  await withApp(reportedRoom(), async () => {
...
test("App files nothing when the ratify target is retired", async () => {
  await withApp(reportedRoom({ retired: true }), async () => {
    const agree = await acceptControl();
    ...
    // must be rewritten rather than passing for a new reason.
    assert.ok(agree, "canRatify does not read retirement, so the control is expected to be offered here");
    await click(agree);
...
test("App files nothing when the ratify target was never ruled effective", async () => {
  await withApp(reportedRoom({ verdict: "ineffective" }), async () => {
    const agree = await acceptControl();
    assert.ok(agree, "canRatify does not read the decision, so the control is expected to be offered here");
    await click(agree);
