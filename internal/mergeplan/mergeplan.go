...
func PlanSuccession(projection workroom.Projection, changes []Change, candidates map[string]Candidate) Succession {
	...
	retire := map[string]string{}
	var leftLive map[string]LeftLive
	if candidates != nil {
	...
	leaveLive := func(artifact workroom.Artifact) bool {
		if candidate, classified := candidates[artifact.Event]; classified &&
			(candidate.Class == ClassCarried || candidate.Class == ClassProtectedSibling || candidate.Class == ClassAbandoned) {
...
const (
	...
	AuthorizationTrailer             = "Gitseq-Authorization: "
...
func ReadReceipt(ctx context.Context, checkout, head string) (Receipt, bool, error) {
	...
	for _, line := range strings.Split(message, "\n") {
		...
		case strings.HasPrefix(line, AuthorizationTrailer):
			receipt.Authorization = strings.TrimPrefix(line, AuthorizationTrailer)
		case strings.HasPrefix(line, AuthorizationRatificationTrailer):
			receipt.AuthorizationRatification = strings.TrimPrefix(line, AuthorizationRatificationTrailer)
