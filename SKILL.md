...
11. **Ordinary staleness is not a question.** A request, promise or
    ...
    requirements did not. Its author re-anchors it on the live successor
    and carries on — retire it with a successor that rests on the current
    basis, or, for a promise or report, leave it, since `gs merge` records
    ordinary staleness and lands anyway. Do not file a request asking
    whether the work is still wanted; nobody needs to decide again what
    was already decided. Re-ask only when something that bears on the
    decision actually changed: a condition of satisfaction, the
    addressee's availability, or the governing decision itself retired
    with no successor. Nine days of a stale queue is a bookkeeping debt,
