...
func TestRevListMetadataReturnsOrderedCommitIdentity(t *testing.T) {
	...
	if metadata[0].OID != string(first) || metadata[0].Tree != tree || len(metadata[0].Parents) != 0 || metadata[0].Message != "first\n\nbody\x1e\n" {
		t.Fatalf("first metadata = %+v", metadata[0])
	}
	if metadata[1].OID != string(second) || metadata[1].Tree != tree || len(metadata[1].Parents) != 1 || metadata[1].Parents[0] != string(first) || metadata[1].Message != "second\n" {
