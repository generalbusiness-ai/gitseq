...
const (
	gsMain  = "spike/cmd/gs/main.go"
	mcpMain = "spike/cmd/gitseq-mcp/main.go"
)
	...
	ast.Inspect(function, func(node ast.Node) bool {
		...
		for _, element := range literal.Elts {
			...
			case "name":
				...
				found = true
			case "inputSchema":
				arguments, required, ok := schemaFields(pair.Value)
				if !ok {
					walkErr = fmt.Errorf("tool input schema is not an object(...) call")
					return false
				}
...
// schemaFields reads argument names out of an `object(map[string]any{...}, required...)`
// call, which is how the adapter builds every tool's input schema.
func schemaFields(expression ast.Expr) ([]string, []string, bool) {
	...
	if literal, ok := call.Args[0].(*ast.CompositeLit); ok {
		for _, element := range literal.Elts {
			...
			if key, ok := stringValue(pair.Key); ok {
