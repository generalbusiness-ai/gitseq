...
func reviewRequestCommand(ctx context.Context, arguments []string) error {
	...
	if set.NArg() != 0 {
		return errors.New("review-request takes no positional arguments")
	}
	if *head == "" || *to == "" {
		return errors.New("review-request requires --head and --to: gs review-request --head <sha> --to <actor>")
	}
