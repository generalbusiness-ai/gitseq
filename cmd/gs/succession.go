...
func planSuccession(projection workroom.Projection, changes []mergeChange, candidates map[string]successionCandidate) successionPlan {
	...
	landed := func(path string) {
		// Git proves this exact path changed. A covering directory is a separate
		// artifact lineage: it remains current in the target world and is carried
		// in the receipt rather than replaced by this exact-path successor.
		published[path] = true
...
// when its commit is in the target's world and its exact lineage is changed.
// An in-target wider pointer remains live as carried accounting; the reviewed
// candidate follows the same rule. Every outside-world candidate is left live
// as either a protected sibling or abandoned candidate.
