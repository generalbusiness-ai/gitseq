...
| An authorization or release report is filed whose `target_ref` no longer holds its `target_pre_head` | Refused by [`gs state`](state.md) when the report is written, not left for the merge to discover. An exact retry of a report already accepted replays that report instead of being measured again. `remeasure=disjoint-paths` relaxes this to an ancestry check. |
