...
- the admitted-time authority rule, and its projection. A ratification is
  decided by the satisfier on the target kind's definition **as that definition
  stood when the target statement was admitted**, never by whichever definition
  of the kind governs now. The fold has always decided this way — the
  definition is bound to the record at admission and read back from there — and
  the rule is the same one `lifecycle` already follows, for the same reason: a
  kind redefined later must not change what earlier records mean or what may be
  done to them. What changes here is that the fold now publishes that captured
  satisfier per statement instead of keeping it to itself. It has to, because
  the value is not recoverable from what a reader is given: the captured
  definition is not in the projection, and reconstructing it would mean
  replaying every kind definition and its ratifications in reading order —
  this layer's authority rule, rebuilt outside this layer. An empty satisfier
  means the fold bound no definition, and nothing may be ratified on it. This
  projection change advances the profile to `workroom-fold@13`, so a cache
  written under `@12` is rejected and the history replayed; without that gate a
...
- **Prefilled citations are a convenience, not a claim.** Where a record the
  ...
  depends on serialisation order, and bounded by a fixed limit so no screen can
...
- **Widened since admission.** The screen offers an act the fold refuses.
  ...
  append-only log for ever, saying somebody tried to do something they were
  never allowed to do — and they were told the act was available.

This paragraph previously stated the live-vocabulary rule, and the browser
implemented it. The page and the code agreed with each other and both diverged
