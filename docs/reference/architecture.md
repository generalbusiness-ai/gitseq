...
- guarded review and merge semantics, including the merge receipt that lets the
  ...
  the paths that approval reviewed — those where the implementer stood an
  artifact at the approved head before the reviewer signed — since the fold is
  pure over records and can verify no merge head, diff, or tree;
