...
| `SKILL.md` | Application guidance | Governs agent conduct in Workroom, including the three conclusions every implementation review records before approval: architecture, security and simplification. It is not a kernel protocol specification. |
...
not do both. The architecture conclusion is one of three that `SKILL.md`
requires before approval; the other two are security across the affected
boundaries, and any opportunity to achieve the same result more simply.
`SKILL.md` carries all three, because they are how any workroom reviews an
implementation. `AGENTS.md` carries only what is local to this repository.
