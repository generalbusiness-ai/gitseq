...
**The newest live completion record wins.** Conforming artifacts and explicit
reports are read together in event order, and retired ones are skipped. An
artifact conforms when its promisor authored it, it names a commit, and it
rests on exactly one promise: the promise it reports.
